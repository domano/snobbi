...
export default function FileInput({register, name}) {
    let [previewImage, setPreviewImage] = useState(null)
    const handleChange = (e) => setPreviewImage(URL.createObjectURL(e.target.files[0]))
    return <div
            ...
            <div
                ...
                <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-600">
                        <label htmlFor="image" onChange={handleChange}
                               className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                            <span>Upload a file</span>
                            <input id={name} name={name} ref={register({required: true})} type="file"
                                   className="sr-only"
                            />
                        </label>
